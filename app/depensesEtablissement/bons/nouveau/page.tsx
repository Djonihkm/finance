...
const NouveauBonPage = () => {
  ...
  const [lignes, setLignes] = useState<LigneCommande[]>([
    { designation: 'Poutres en chêne massif (H=2.40m)', quantite: 12, prixUnitaire: 450 },
  ]);
  ...
  <div className="w-20 h-20 relative shrink-0">
    ...
    />
                ...
                <div className="text-xs text-gray-600 text-right leading-relaxed">
                  ...
                  <p>12<sup>e</sup> arrondissement, Commune de Cotonou</p>
                ...
                <div className="space-y-4">
                  <div className="border-l-4 border-[#11355b] pl-4">
                    ...
                    <input
                      ...
                      onChange={(e) => handleBonChange('numero', e.target.value)}
                  ...
                  <div className="border-l-4 border-[#11355b] pl-4">
                    ...
                    <input
                      ...
                      onChange={(e) => handleBonChange('dateExpedition', e.target.value)}
                      className="text-base font-semibold text-[#11355b] bg-transparent w-full focus:outline-none focus:bg-blue-50/50 rounded px-1 -ml-1"
                    />
                  </div>
                </div>
                ...
                <table className="w-full border-collapse">
                  <thead>
                    <tr className="bg-[#11355b] text-white">
                      <th className="px-4 py-3 text-left text-[10px] font-bold uppercase tracking-wider">Désignation</th>
                      <th className="px-4 py-3 text-center text-[10px] font-bold uppercase tracking-wider w-24">Quantité</th>
                      <th className="px-4 py-3 text-right text-[10px] font-bold uppercase tracking-wider w-32">Prix unitaire</th>
                      <th className="px-4 py-3 text-right text-[10px] font-bold uppercase tracking-wider w-32">Total</th>
                    ...
                    {lignes.map((ligne, index) => (
                      <tr key={index} className="border-b border-gray-100 hover:bg-gray-50/50">
                        ...
                        <td className="px-4 py-3 text-right text-sm font-bold text-[#11355b]">
                          {(ligne.quantite * ligne.prixUnitaire).toLocaleString('fr-FR', { minimumFractionDigits: 2 })}
                        </td>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6 pt-6 border-t border-gray-100">
                ...
                <div className="bg-gray-50 rounded-lg p-5 space-y-3">
                  <div className="flex justify-between items-center text-sm">
                    <span className="text-gray-500 uppercase text-xs font-bold tracking-wider">Sous-total</span>
                    <span className="font-semibold text-[#11355b]">{sousTotal.toLocaleString('fr-FR', { minimumFractionDigits: 2 })}</span>
                  </div>
                  ...
                  <div className="flex justify-between items-center text-sm">
                    ...
                    <span className="font-semibold text-[#11355b]">{tvaMontant.toLocaleString('fr-FR', { minimumFractionDigits: 2 })}</span>
                  </div>

                  <div className="flex justify-between items-center text-sm">
                    <span className="text-orange-500 uppercase text-xs font-bold tracking-wider">Promo / Remise</span>
                    <span className="font-semibold text-orange-500">- {bonData.promo.toLocaleString('fr-FR', { minimumFractionDigits: 2 })}</span>
                  </div>

                  <div className="flex justify-between items-center pt-3 border-t border-gray-200">
                    <span className="text-base font-bold text-[#11355b] uppercase tracking-wide">Total TTC</span>
                    <span className="text-xl font-bold text-[#11355b]">{totalTTC.toLocaleString('fr-FR', { minimumFractionDigits: 2 })}</span>
                  </div>
                </div>
                ...
                <div>
                  ...
                  <textarea
                    ...
                    onChange={(e) => handleBonChange('signature', e.target.value)}
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    <div>
                      <p className="text-[10px] font-bold text-gray-400 uppercase tracking-wider mb-1">Fait à</p>
                      <input
                        type="text"
                        value={bonData.faitA}
                    ...
                    <div>
                      <p className="text-[10px] font-bold text-gray-400 uppercase tracking-wider mb-1">La date</p>
                      <input
                        type="date"
                        value={bonData.laDate}
                        onChange={(e) => handleBonChange('laDate', e.target.value)}
                        className="w-full text-sm text-gray-700 border-b border-gray-300 bg-transparent focus:outline-none focus:border-[#11355b] py-1"
                      />
                    </div>
                  </div>
        ...
        <div className="fixed inset-0 bg-black/30 backdrop-blur-[2px] flex items-center justify-center z-50 p-4 animate-fadeIn">
          <div className="bg-white rounded-2xl shadow-2xl max-w-md w-full p-8 text-center animate-scaleIn">
            ...
            <p className="text-sm text-gray-500 mb-6">
              Le bon de commande <span className="font-bold text-[#11355b]">{bonData.numero}</span> a été soumis avec succès.
            </p>
