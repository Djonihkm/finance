...
import { ArrowLeft, Ban, CheckCircle, AlertTriangle, Mail, Phone, Briefcase, Building2, Calendar } from "lucide-react";
import { toast } from "sonner";
import type { UserDetail } from "@/lib/queries";
import { formatRole, formatDate } from "@/lib/utils/formatters";
...
export default function UtilisateurDetailView({ data }: { data: UserDetail }) {
  const router = useRouter();

  const handleToggleStatut = async () => {
    const next = !data.isActive;
    const label = next ? "Réactiver" : "Suspendre";
    if (!confirm(`${label} le compte de ${data.nom} ${data.prenom} ?`)) return;
    try {
      ...
      toast.success(`${data.nom} ${data.prenom} ${next ? "réactivé(e)" : "suspendu(e)"}.`);
      router.refresh();
    } catch {
  ...
  const initiales = `${data.nom[0] ?? ""}${data.prenom[0] ?? ""}`.toUpperCase();
    ...
    <div className="max-w-7xl mx-auto">
      ...
      <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-6 mb-5 flex flex-col sm:flex-row sm:items-center gap-5">
        ...
        <div className="flex-1 min-w-0">
          <h1 className="text-2xl font-bold text-[#11355b] mb-1">{data.nom} {data.prenom}</h1>
      ...
      <div className="grid grid-cols-1 md:grid-cols-[1fr_260px] gap-5">
        ...
        <div className="space-y-4 lg:sticky lg:top-6 self-start">
          ...
          <div className="bg-[#11355b] rounded-xl p-5 text-white">
            ...
            <p className="text-2xl font-bold mb-1">{formatRole(data.role)}</p>
