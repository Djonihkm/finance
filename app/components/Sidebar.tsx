  ...
  const renderMenuItem = (item: MenuItem) => {
    ...
    return (
      <Link
        ...
        `}
      >
        <span className={`flex-shrink-0 transition-colors ${isActive ? "text-yellow-400" : "text-blue-300 group-hover:text-white"}`}>
          {item.icon}
        </span>
    ...
    <>
      ...
      >
        ...
        <nav className="flex-1 overflow-y-auto py-6 px-3 space-y-4 custom-scrollbar">
          ...
          {showLabels
            ? <h3 className="px-4 text-xs font-bold text-blue-400 uppercase tracking-wider pt-2">Finances & Gestion</h3>
            : <div className="w-8 h-px bg-white/20 mx-auto my-2" />
          }

          <div className="space-y-1">{financeMenuItems.map(renderMenuItem)}</div>
        </nav>

        {/* Footer utilisateur */}
        <div className="p-4 border-t border-white/10 bg-black/20">
          <div className={`flex items-center gap-3 p-2 rounded-lg hover:bg-white/5 cursor-pointer transition-colors ${!showLabels ? "justify-center" : ""}`}>
